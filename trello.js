  ...
  tdv.buildDependencyTree = function(boardName, epics, cards) {
    ...
    cards.forEach(card => {
      if (!epicIdsShortcuts.hasOwnProperty(card.id)) {
        ...
        if (typeof link === 'undefined') {
          links.push({source: card.name, target: orphanName, url: card.url})
        } else {
          var target = cardsIdsShortcuts[link];
          if (typeof target !== 'undefined') {
            links.push({source: card.name, target: target.name, url: card.url})
          }
        }
      }
    });
